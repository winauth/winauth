...
namespace WinAuth
{
  ...
  public class BattleNetAuthenticator : Authenticator
  {
    ...
    /// <summary>
    /// We can check if the restore code is valid and remember so don't have to do it again
    /// </summary>
    ...
    public void Restore(string serial, string restoreCode)
    {
      ...
      catch (WebException we)
      {
        ...
        {
          throw new InvalidRestoreResponseException(string.Format("No response from server ({0}). Perhaps maintenance?", code));
        }
        else
        {
    ...
    /// <summary>
    /// Get the base mobile url based on the region
    /// </summary>
